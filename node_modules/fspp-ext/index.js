...
const fsppext = Object.assign({}, fspp, {
  ...
  /**
   * Search a certain item recursively under rootdir.
   *
   * @param {string} itemName The target item name.
   * @param {string} [rootdir] The root directory path to start the search with.
   *
   * @exception {Promise.reject<Error>} when any the subdirectory is unaccessible
   *
   * @async
   * @return {Promise<string|false>} Returns the full path of the first item found, false otherwise.
   */
  async pathSearch(itemName, rootdir = process.cwd()) {
    const subSearchCall = async dir => {
      if ((await fspp.lstat(dir)).isDirectory())
        for (const item of await fspp.readdir(dir)) {
          const ret =
            item === itemName ? path.join(dir, item) : await subSearchCall(path.join(dir, item));
          if (ret) return ret;
        }
      return false;
    };
    await fspp.access((rootdir = path.resolve(rootdir)));
    return subSearchCall(rootdir);
  },
  /**
   ...
   * @param {string} [rootdir] The root directory path to start the search with.
   *
   * @exception {Error} when any the subdirectory is unaccessible
   *
   * @async
   * @return {string|false} Returns the full path of the first item found, false otherwise.
   */
  pathSearchSync(itemName, rootdir = process.cwd()) {
    const subSearchCallSync = dir => {
      if (fspp.lstatSync(dir).isDirectory())
        for (const item of fspp.readdirSync(dir)) {
          const ret =
            item === itemName ? path.join(dir, item) : subSearchCallSync(path.join(dir, item));
          if (ret) return ret;
        }
      return false;
    };
    fspp.accessSync((rootdir = path.resolve(rootdir)));
    return subSearchCallSync(rootdir);
  },
});
