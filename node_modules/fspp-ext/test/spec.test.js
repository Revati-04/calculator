...
const TEST_SANDBOX_ROOT = path.resolve(`test/sandbox`),
  ...
  FORBIDEN_PATH = IS_WINDOWS ? `C:/Windows/System32/0000` : `/dev/root`;
...
describe(`fspp-ext`, () => {
  ...
  describe('pathSearch Spec', () => {
    const DEEP_DIR = path.join(TEST_SANDBOX_ROOT, `Search/Level2/Level3`);
    const FAKE_DIR1 = path.join(TEST_SANDBOX_ROOT, `Search/Level3/Level2`);
    const FAKE_DIR2 = path.join(TEST_SANDBOX_ROOT, `Search/Level3/Level1`);
    const TESTFILE = path.join(DEEP_DIR, `cat45.txt`);

    beforeEach(() => {
      fs.ensureDirSync(DEEP_DIR);
      fs.ensureDirSync(FAKE_DIR1);
      fs.ensureDirSync(FAKE_DIR2);
      fs.writeFileSync(TESTFILE, `I'm a cat.`, `utf8`);
    });

    it('Should pick up correct path for test file', async () => {
      const p = await fs.pathSearch(path.basename(TESTFILE), TEST_SANDBOX_ROOT);
      assert.equal(p, TESTFILE, `pathSearch failed to find path to the test file.`);
    });

    it('Should pick up correct path for hosts file - synced version', () => {
      const p = fs.pathSearchSync(path.basename(TESTFILE), TEST_SANDBOX_ROOT);
      assert.equal(p, TESTFILE, `pathSearchSync failed to find path to the test file.`);
    });
  });
});
